	...
	<div class="max-w-2xl mx-auto px-4 py-8">
		...
		<div class="flyer bg-white border-4 rounded-2xl p-10 text-center {item.type === 'lost' ? 'border-[var(--color-lost)]' : 'border-[var(--color-found)]'}">
			...
			<div class="mt-8 flex items-center justify-center gap-2">
				<img src="/logo-v2-pin-checkmark.svg" alt="Fundið" class="w-5 h-7" />
				<span class="text-sm font-semibold text-[var(--color-ink)]">Fundið</span>
				<span class="text-xs text-[var(--color-muted)]">· {$t('common.tagline')}</span>
			</div>
		</div>
	</div>
{/if}
