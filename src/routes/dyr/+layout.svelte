...
<div class="min-h-screen flex flex-col bg-[var(--color-surface)]">
	<header class="border-b border-[var(--color-border)] bg-white sticky top-0 z-50">
		<nav class="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
			<a href="/dyr" class="flex items-center gap-2">
				...
				<span class="text-xs font-medium text-[var(--color-amber)] bg-[var(--color-amber-light)] px-2 py-0.5 rounded-full">Dýr</span>
			</a>
	...
	<footer class="border-t border-[var(--color-border)] py-8 text-center space-y-2">
		<p class="text-sm text-[var(--color-muted)]">Fundið Dýr — {$t('common.tagline')}</p>
		<div class="flex justify-center gap-3">
			<a href="/" class="text-xs text-[var(--color-muted)] hover:text-[var(--color-ink)] transition-colors">fundid.is</a>
		</div>
	</footer>
</div>
